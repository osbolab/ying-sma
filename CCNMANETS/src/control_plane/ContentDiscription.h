...
{
  enum  META_TYPE  {PublishTime=0, Category=1, Location=2, Rank=3, Invalid=4};
  static const char * META_TYPE_STR[] = { "PublishTime", "Category", "Location", "Rank" };
  inline META_TYPE to_META_TYPE(const char* str)
  {
    if (strcmp(str, "PublishTime") == 0)
      return PublishTime;
...
public:
  ...
  void addNewChunk(unsigned int seqNo, std::string chunkID);  // only owner can do that
  void delChunk(unsigned int seqNo);     // only owner can do that
  void addAttribute(ContentAttribute::META_TYPE type, std::string typeValue);   // only owner can do that
  void delAttribute(ContentAttribute::META_TYPE type);   // only owner can do that
  ...
  void print() const;   // for unit test, should be exported in JSON format for network use
