...
ContentDiscription::ContentDiscription (std::string filename) : contentName (filename) {}
...
void ContentDiscription::addAttribute(ContentAttribute::META_TYPE type, std::string typeValue)
{
...
void ContentDiscription::delChunk (unsigned int seqNo)
{
  ...
  if ( iter != chunkList.end())    chunkList.erase(iter);
}

void ContentDiscription::delAttribute(ContentAttribute::META_TYPE type)
{
  std::map<ContentAttribute::META_TYPE, std::string>::iterator iter = attriList.find(type);
  if ( iter != attriList.end())    attriList.erase(iter);
}
...
{
  std::map<unsigned int, std::string>::const_iterator iter = chunkList.find(seqNo);
  return ((iter == chunkList.end()) ? "" : iter->second);
}

std::string ContentDiscription::getTypeValue(ContentAttribute::META_TYPE type) const
{
  std::map<ContentAttribute::META_TYPE, std::string>::const_iterator iter = attriList.find(type);
...
void ContentDiscription::print() const
{
  ...
  std::map<unsigned int, std::string>::const_iterator chunk_iter = chunkList.begin();
  while (chunk_iter != chunkList.end())
  {
    std::cout << '\t' << chunk_iter->first << " : " << chunk_iter->second << std::endl;
    chunk_iter++;
  }
  std::map<ContentAttribute::META_TYPE, std::string>::const_iterator attri_iter = attriList.begin();
  while (attri_iter != attriList.end())
  {
    ContentAttribute::META_TYPE enumValue = attri_iter->first;
    std::cout << '\t' << ContentAttribute::META_TYPE_STR[enumValue] << " : " << attri_iter->second << std::endl;
    attri_iter++;
  }
  std::cout << std::endl;
}
...
std::vector<std::string> ContentDiscription::getChunkList() const
{
  ...
  {
    std::map<unsigned int, std::string>::const_iterator iter = chunkList.find(i);
...
std::map<ContentAttribute::META_TYPE, std::string> ContentDiscription::getMetaPairList() const
{
  return attriList;
}
...
{
  //must make sure the timestamp field always exists when creating the meta data.
//  std::string time1Str = this->getTypeValue(ContentAttribute::PublishTime);
//  std::string time2Str = discription.getTypeValue(ContentAttribute::PublishTime);
...
std::time_t ContentDiscription::ContentDiscription::timestampToTimeT() const
{
  ...
  struct std::tm when; 
  iss >> std::get_time (&when, "%Y/%m/%d %T"); //format depends on the locale
