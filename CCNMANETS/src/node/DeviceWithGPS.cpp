...
DeviceWithGPS::DeviceWithGPS(std::string id) : deviceID(id), network (NULL), controlPlane(id)
{
...
{
  if (network == NULL && networkToAttach)
  {  
...
{
  if (networkAttached)  networkAttached->dropDevice(this);
}
...
void DeviceWithGPS::beaconing()
{
  ...
  while (true)
  {
    ...
    std::string gpsBroadCastData = getJsonGPS(); //create Json-formatted beaconing message
...
void DeviceWithGPS::broadcastDirectory()
{
  ...
  int numOfEntries = 5;   // Temp solution: The number should be provided by the profiling module.
  while (true)
  {
    if (network == NULL)  break;
...
void DeviceWithGPS::processNeighborQuery () const
{
  ...
  std::cout << "The neighbor records of " << this->getDeviceID() << " is:" << std::endl;
  while (iter != ids.end())
  {
...
void DeviceWithGPS::publishContent(std::string inFileName, std::string outFileName)
{
  ...
  oss << std::put_time(gmtm, "%Y/%m/%d %T");
  attri_pair.push_back(std::make_pair(ContentAttribute::PublishTime, oss.str()));
  controlPlane.publishContent(inFileName, outFileName, attri_pair);
}

void DeviceWithGPS::retrieveContentAs(std::string fileName, std::string exportFileName)
{
...
{
  Json::Value result;
  std::vector<ContentDiscription> directory = controlPlane.getContentDirectory(numOfEntries);
  ...
  while (iter != directory.end())
  {
    ...
    // add attribute list
    Json::Value metas;
    std::map<ContentAttribute::META_TYPE, std::string> metaPairs = iter->getMetaPairList();
    std::map<ContentAttribute::META_TYPE, std::string>::iterator attri_iter = metaPairs.begin();
