...
#endif
// Unix
#if ((_ELPP_OS_LINUX || _ELPP_OS_MAC || _ELPP_OS_FREEBSD) &&                   \
     (!_ELPP_OS_WINDOWS))
#define _ELPP_OS_UNIX 1
#endif    // ((_ELPP_OS_LINUX || _ELPP_OS_MAC || _ELPP_OS_FREEBSD) &&
          // (!_ELPP_OS_WINDOWS))
// Android
...
#define ELPP_ASSERT(expr, msg)                                                 \
  if (!(expr)) {                                                               \
    ...
    _ELPP_INTERNAL_DEBUGGING_OUT_ERROR                                         \
        ...
        << "] WITH MESSAGE \""                                                 \
        << _ELPP_INTERNAL_DEBUGGING_MSG(internalInfoStream.str()) << "\""      \
...
#define _ELPP_INTERNAL_DEBUGGING_WRITE_PERROR                                  \
  {                                                                            \
    ...
    _ELPP_INTERNAL_DEBUGGING_OUT_ERROR << ": " << buff << " [" << errno        \
                                       << "]";                                 \
  }                                                                            \
  (void) 0
#else
#define _ELPP_INTERNAL_DEBUGGING_WRITE_PERROR                                  \
  _ELPP_INTERNAL_DEBUGGING_OUT_ERROR << ": " << strerror(errno) << " ["        \
                                     << errno << "]";                          \
  (void) 0
...
#define ELPP_INTERNAL_INFO(lvl, msg)                                           \
  {                                                                            \
    if (lvl <= _ELPP_INTERNAL_INFO_LEVEL) {                                    \
      std::stringstream internalInfoStream;                                    \
      internalInfoStream << "<INFO> " << msg;                                  \
      _ELPP_INTERNAL_DEBUGGING_OUT_INFO                                        \
          << _ELPP_INTERNAL_DEBUGGING_MSG(internalInfoStream.str())            \
          << _ELPP_INTERNAL_DEBUGGING_ENDL;                                    \
    }                                                                          \
  }
...
// Compiler specific support evaluations
    ...
    defined(_ELPP_FORCE_USE_STD_THREAD)
...
#if _ELPP_COMPILER_GCC || _ELPP_COMPILER_CLANG || _ELPP_COMPILER_INTEL ||      \
    (_ELPP_COMPILER_MSVC && _MSC_VER >= 1800)
#define _ELPP_VARIADIC_TEMPLATES_SUPPORTED 1
#endif    // _ELPP_COMPILER_GCC || _ELPP_COMPILER_CLANG || _ELPP_COMPILER_INTEL
          // || (_ELPP_COMPILER_MSVC && _MSC_VER >= 1800)
// Logging Enable/Disable macros
#if (!defined(_ELPP_DISABLE_LOGS))
#define _ELPP_LOGGING_ENABLED 1
#endif    // (!defined(_ELPP_DISABLE_LOGS))
#if (!defined(_ELPP_DISABLE_DEBUG_LOGS) && (_ELPP_LOGGING_ENABLED) &&          \
     ((defined(_DEBUG)) || (!defined(NDEBUG))))
#define _ELPP_DEBUG_LOG 1
#else
#define _ELPP_DEBUG_LOG 0
#endif    // (!defined(_ELPP_DISABLE_DEBUG_LOGS) && (_ELPP_LOGGING_ENABLED) &&
          // ((defined(_DEBUG)) || (!defined(NDEBUG))))
...
#error                                                                         \
    "Easylogging++ 9.0+ is only compatible with C++0x (or higher) compliant compiler"
...
#define ELPP_COUT_LINE(logLine) logLine << std::flush
  ...
  ///
  /// @detail This prevents initializing class making it static unless an
  /// explicit constructor is declared.
...
/// regardless of
/// what the severity is. Or they can choose to log using hierarchical logging
/// flag
...
namespace base
{
  ...
  namespace consts
  {
    ...
    // Date/time
    static const char* kDays[7] = {"Sunday",
                                   "Monday",
                                   "Tuesday",
                                   "Wednesday",
                                   "Thursday",
    ...
    static const char* kMonths[12] = {"January",
                                      "February",
                                      "March",
                                      "Apri",
                                      "May",
                                      ...
                                      "November",
                                      "December"};
    static const char* kMonthsAbbrev[12] = {"Jan",
                                            "Feb",
                                            "Mar",
                                            "Apr",
                                            "May",
    ...
    const struct {
      ...
      const char* detail;
        ...
        {SIGILL,
         ...
         "Illegal instruction",
         "Generally due to a "
         "corruption in the code or "
         "to an attempt to execute "
         "data."},
        {SIGSEGV,
         "SIGSEGV",
         "Invalid access to memory",
         "Program is trying to read an invalid (unallocated, deleted or "
...
namespace base
{
  ...
  /// @brief A milliseconds width class containing actual width and offset for
  /// date/time
  ...
  /// @brief Namespace containing utility functions/static classes used
  /// internally
  namespace utils
  {
    ...
    /// @brief Gets value of const char* but if it is nullptr, a string nullptr
    /// is returned
    ...
    static inline void abort(int status,
                             const std::string& reason = std::string())
    {
      // Both status and reason params are there for debugging with tools like
      // gdb etc
      _ELPP_UNUSED(status);
      _ELPP_UNUSED(reason);
    ...
    /// Flag_T and returns value after bitwise operation
    /// Use these function as <pre>flag = bitwise::Or<MyEnum>(MyEnum::val1,
    /// flag);</pre>
    namespace bitwise
    {
  ...
  namespace threading
  {
    ...
    /// @brief Gets ID of currently running threading in windows systems. On
    /// unix, nothing is returned.
  ...
  namespace utils
  {
    ...
    public:
      ...
      static bool createPath(const std::string& path)
      {
        ...
        while (currPath != nullptr) {
          ...
          currPath =
              STRTOK(nullptr, base::consts::kFilePathSeperator, &nextTok_);
#endif    // _ELPP_OS_UNIX
        }
        if (status == -1) {
          ELPP_INTERNAL_ERROR("Error while creating path [" << path << "]",
                              true);
          return false;
        }
        return true;
    ...
    /// @brief String utilities helper class used internally. You should not use
    /// it.
    class Str : base::StaticClass
    {
    public:
      /// @brief Checks if character is digit. Dont use libc implementation of
      /// it to prevent locale issues.
      ...
      /// @brief Replaces all instances of replaceWhat with 'replaceWith'.
      /// Original variable is changed for performance.
      ...
      /// @brief Replaces all instances of 'replaceWhat' with 'replaceWith'.
      /// (String version) Replaces in place
      ...
      static void replaceFirstWithEscape(
      ...
      /// @brief Compares cstring equality (case-insensitive) - uses
      /// toupper(char)
      ...
      static inline char* convertAndAddToBuff(std::size_t n,
                                              int len,
                                              char* buf,
                                              const char* bufLim,
      ...
      {
        STRCPY(buff, "", lim);
        _ELPP_UNUSED(lim);    // For *nix we dont have anything using lim in
                              // above STRCPY macro
        return buff;
      }
    ...
    /// @brief Operating System helper static class used internally. You should
    /// not use it.
    ...
    public:
      ...
      ///        We are not using <code>getenv(const char*)</code> because of
      ///        CRT deprecation
      ...
      ///
      /// @detail This is applicable only on unix based systems, for all other
      /// OS, an empty string is returned.
      ...
      /// @brief Gets environment variable. This is cross-platform and CRT safe
      /// (for VC++)
      /// @param variableName Environment variable name
      /// @param defaultVal If no environment variable or value found the value
      /// to return by default
      /// @param alternativeBashCommand If environment variable not found what
      /// would be alternative bash command
      ...
      static inline std::string currentUser(void)
      {
        ...
        return getEnvironmentVariable(
            "USER", base::consts::kUnknownUser, "whoami");
#elif _ELPP_OS_WINDOWS
      ...
      ///
      /// @detail For android systems this is device name with its manufacturer
      /// and model seperated by hyphen
      static inline std::string currentHost(void)
      {
#if _ELPP_OS_UNIX && !_ELPP_OS_ANDROID
        return getEnvironmentVariable(
            "HOSTNAME", base::consts::kUnknownHost, "hostname");
#elif _ELPP_OS_WINDOWS
...
#define _ELPP_INITI_BASIC_DECLR                                                \
  ...
  {                                                                            \
    ...
    /// @brief Contains utilities for cross-platform date/time. This class make
    /// use of el::base::utils::Str
    class DateTime : base::StaticClass
    {
    public:
      /// @brief Cross platform gettimeofday for Windows and unix platform. This
      /// can be used to determine current millisecond.
      ///
      /// @detail For unix system it uses gettimeofday(timeval*, timezone*) and
      /// for Windows, a seperate implementation is provided
      ...
      /// @param msWidth A pointer to base::MillisecondsWidth from configuration
      /// (non-null)
      ...
      /// @brief Formats time to get unit accordingly, units like second if >
      /// 1000 or minutes if > 60000 etc
      ...
      {    // NOLINT
        ...
        for (base::type::EnumType i = start;
             i < base::consts::kTimeFormatsCount - 1;
             ++i) {
    ...
    private:
      static inline struct ::tm* buildTimeInfo(struct timeval* currTime,
                                               struct ::tm* timeInfo)
      {
#if _ELPP_OS_UNIX
        ...
        return timeInfo;
#else
        // For any other compilers that don't have CRT warnings issue e.g, MinGW
        // or TDM GCC- we use different method
    ...
    /// el::Helpers::setArgs(..) or _START_EASYLOGGINGPP(..)
    ...
    public:
        ...
        for (int i = 1; i < m_argc; ++i) {
          ...
          if (v != nullptr && strlen(v) > 0) {
            ...
            if (hasParamWithValue(key.c_str())) {
              ELPP_INTERNAL_INFO(
                  1,
                  "Skipping [" << key << "] arg since it already has value ["
          ...
          if (v == nullptr) {
            if (hasParam(m_argv[i])) {
              ELPP_INTERNAL_INFO(
                  1,
                  "Skipping [" << m_argv[i] << "] arg since it already exists");
            } else {
      ...
      /// @brief Returns true if arguments contain paramKey with a value
      /// (seperated by '=')
      ...
      {
        return std::find(m_params.begin(),
                         m_params.end(),
    ...
    /// @brief Abstract registry (aka repository) that provides basic interface
    /// for pointer repository specified by T_Ptr type.
    ///
    /// @detail Most of the functions are virtual final methods but anything
    /// implementing this abstract class should implement
    /// unregisterAll() and deepCopy(const AbstractRegistry<T_Ptr, Container>&)
    /// and write registerNew() method according to container
    /// and few more methods; get() to find element, unregister() to unregister
    /// single entry.
    /// Please note that this is thread-unsafe and should also implement
    /// thread-safety mechanisms in implementation.
    ...
    ///         of AbstractRegistry<T_Ptr, Container>. Any implementation of
    ...
    public:
      ...
      /// @brief Copy constructor that is useful for base classes. Try to avoid
      /// this constructor, use move constructor.
      ...
      /// @brief Assignment operator that unregisters all the existing
      /// registeries and deeply copies each of repo element
      /// @see unregisterAll()
    ...
    protected:
      ...
      /// @brief Gets pointer from repository. If none found, nullptr is
      /// returned.
    ...
    /// AbstractRegistry<T_Ptr, Container>. Any implementation of this class
    ...
    public:
      ...
      typedef
          typename RegistryWithPred<T_Ptr, Pred>::const_iterator const_iterator;
      ...
      {
        for (const_iterator it = sr.list().begin(); it != sr.list().end();
             ++it) {
    ...
    protected:
      ...
      /// @brief Gets pointer from repository with speicifed arguments.
      /// Arguments are passed to predicate
...
namespace base
{
  /// @brief Represents log format containing flags and date format. This is
  /// used internally to start initial log
      ...
      auto conditionalAddFlag =
        ...
        while ((foundAt = formatCopy.find(specifier, foundAt + 1)) !=
          ...
          if (foundAt > 0 &&
            ...
            if (hasFlag(flag)) {
              // If we already have flag we remove the escape chars so that '%%'
              // is turned to '%'
              // even after specifier resolution - this is because we only
              // replaceFirst specifier
  ...
  protected:
    ...
    virtual void updateFormatSpec(void) ELPP_FINAL
    {
      ...
      if (hasFlag(base::FormatFlags::User)) {
        ...
        base::utils::Str::replaceFirstWithEscape(
            m_format,
            base::consts::kCurrentUserFormatSpecifier,
            base::utils::s_currentUser);
      }
      if (hasFlag(base::FormatFlags::Host)) {
        base::utils::Str::replaceFirstWithEscape(
            m_format,
            base::consts::kCurrentHostFormatSpecifier,
...
/// string itself, it will be embedded inside quotes
...
public:
  ...
  /// '_ELPP_DEBUG_ASSERT_FAILURE' to make sure you
  ///         do not proceed without successful parse.
  inline bool parseFromFile(const std::string& configurationFile,
                            Configurations* base = nullptr)
  {
    bool assertionPassed = false;
    ELPP_ASSERT(
        (assertionPassed =
             base::utils::File::pathExists(configurationFile.c_str(), true)),
  ...
  /// contents. Make sure all the necessary
  ...
  /// Also note that configuration types
  /// ConfigurationType::MillisecondsWidth and
  /// ConfigurationType::PerformanceTracking will be ignored if not set for
  ...
  /// configuration is, it will always be string
  /// from users' point of view. This is then parsed later to be used
  /// internally.
  /// @see Configuration::setValue(const std::string& value)
  /// @see el::Level
  /// @see el::ConfigurationType
  inline void set(Level level,
                  ConfigurationType configurationType,
                  const std::string& value)
  {
    base::threading::ScopedLock scopedLock(lock());
    ...
    if (level == Level::Global) {
      unsafeSetGlobally(configurationType,
                        value,
  ...
  void setToDefault(void)
  {
    ...
    setGlobally(
        ConfigurationType::PerformanceTracking, std::string("true"), true);
    ...
    set(Level::Debug,
        ...
        std::string("%datetime %level [%logger] [%func] [%fbase:%line] %msg"));
  ...
  /// not exist for.
  /// This function is useful when you want to minimize chances of failures,
  /// e.g, if you have a configuration file that sets
  /// configuration for all the configurations except for Enabled or not, we use
  /// this so that ENABLED is set to default i.e,
  /// true. If you dont do this explicitley (either by calling this function or
  /// by using second param in Constructor
  /// and try to access a value, an error is thrown
  void setRemainingToDefault(void)
  {
    base::threading::ScopedLock scopedLock(lock());
    unsafeSetIfNotExist(
        Level::Global, ConfigurationType::Enabled, std::string("true"));
#if !defined(_ELPP_NO_DEFAULT_LOG_FILE)
...
#endif    // !defined(_ELPP_NO_DEFAULT_LOG_FILE)
    unsafeSetIfNotExist(
        Level::Global, ConfigurationType::ToFile, std::string("true"));
    unsafeSetIfNotExist(Level::Global,
                        ConfigurationType::ToStandardOutput,
                        std::string("true"));
    unsafeSetIfNotExist(
        Level::Global, ConfigurationType::MillisecondsWidth, std::string("3"));
    unsafeSetIfNotExist(Level::Global,
                        ConfigurationType::PerformanceTracking,
                        std::string("true"));
    unsafeSetIfNotExist(
        Level::Global, ConfigurationType::MaxLogFileSize, std::string("0"));
    unsafeSetIfNotExist(Level::Global,
                        ConfigurationType::Format,
                        std::string("%datetime %level [%logger] %msg"));
    unsafeSetIfNotExist(
        ...
        std::string("%datetime %level [%logger] [%func] [%fbase:%line] %msg"));
    // INFO and WARNING are set to default by Level::Global
    unsafeSetIfNotExist(Level::Error,
                        ConfigurationType::Format,
                        std::string("%datetime %level [%logger] %msg"));
    unsafeSetIfNotExist(Level::Fatal,
                        ConfigurationType::Format,
                        std::string("%datetime %level [%logger] %msg"));
    unsafeSetIfNotExist(Level::Verbose,
                        ConfigurationType::Format,
                        std::string("%datetime %level-%vlevel [%logger] %msg"));
    unsafeSetIfNotExist(
        Level::Trace,
  ...
  public:
    ...
    /// @return True if successfully parsed, false otherwise. You may define
    /// '_STOP_ON_FIRST_ELPP_ASSERTION' to make sure you
    ///         do not proceed without successful parse.
    ...
    /// @detail This configuration string has same syntax as configuration file
    /// contents. Make sure all the necessary
    /// new line characters are provided. You may define
    /// '_STOP_ON_FIRST_ELPP_ASSERTION' to make sure you
  ...
  private:
    ...
    static inline bool isConfig(const std::string& line)
    {
      ...
      return line != "" && (line[0] >= 65 || line[0] <= 90 || line[0] >= 97 ||
                            line[0] <= 122) &&
...
namespace base
{
  ...
  /// internally in order to read correct configurations.
  /// This is to perform faster while writing logs using correct configurations.
  ///
  /// This is thread safe and final class containing non-virtual destructor
  /// (means nothing should inherit this class)
  class TypedConfigurations : public base::threading::ThreadSafe
  {
  public:
    /// @brief Constructor to initialize (construct) the object off
    /// el::Configurations
    /// @param configurations Configurations pointer/reference to base this
    /// typed configurations off.
    ...
    {
      return getConfigByVal<bool>(
          level, &m_toStandardOutputMap, "toStandardOutput");
    }

    inline const base::LogFormat& logFormat(Level level)
    {
      return getConfigByRef<base::LogFormat>(
          level, &m_logFormatMap, "logFormat");
    }
    ...
    millisecondsWidth(Level level = Level::Global)
    {
      return getConfigByRef<base::MillisecondsWidth>(
          level, &m_millisecondsWidthMap, "millisecondsWidth");
    }

    inline bool performanceTracking(Level level = Level::Global)
    {
      return getConfigByVal<bool>(
          level, &m_performanceTrackingMap, "performanceTracking");
    }

    inline base::type::fstream_t* fileStream(Level level)
    {
      return getConfigByRef<base::FileStreamPtr>(
                 level, &m_fileStreamMap, "fileStream").get();
    }

    inline std::size_t maxLogFileSize(Level level)
    {
      return getConfigByVal<std::size_t>(
          level, &m_maxLogFileSizeMap, "maxLogFileSize");
    }

    inline std::size_t logFlushThreshold(Level level)
    {
      return getConfigByVal<std::size_t>(
          level, &m_logFlushThresholdMap, "logFlushThreshold");
    }
  ...
  private:
    ...
    template <typename Conf_T>
    inline Conf_T unsafeGetConfigByVal(Level level,
                                       const std::map<Level, Conf_T>* confMap,
                                       const char* confName)
    {
      _ELPP_UNUSED(confName);
      typename std::map<Level, Conf_T>::const_iterator it =
          confMap->find(level);
      if (it == confMap->end()) {
        try {
    ...
    template <typename Conf_T>
    inline Conf_T& unsafeGetConfigByRef(Level level,
                                        std::map<Level, Conf_T>* confMap,
                                        const char* confName)
    {
      _ELPP_UNUSED(confName);
                  ...
                  bool includeGlobalLevel = true)
    {
      ...
      // level
      typename std::map<Level, Conf_T>::iterator it =
    ...
    void build(Configurations* configurations)
    {
      ...
      for (Configurations::const_iterator it = configurations->begin();
        ...
        if (conf->configurationType() == ConfigurationType::Enabled) {
          ...
          setValue(conf->level(), getBool(conf->value()), &m_toFileMap);
                   ...
                   ConfigurationType::ToStandardOutput) {
          setValue(
              conf->level(), getBool(conf->value()), &m_toStandardOutputMap);
          ...
          // ToFile
          // to be false. Because configuring logger is not necessarily
          // performance
          // sensative operation, we can live with another loop; (by the way
          // this loop
      ...
      // As mentioned early, we will now set filename configuration in separate
      ...
      for (std::vector<Configuration*>::iterator conf =
               withFileSizeLimit.begin();
           conf != withFileSizeLimit.end();
           ++conf) {
    ...
    std::string resolveFilename(const std::string& filename)
    {
      ...
      if ((dateIndex = resultingFilename.find(
        ...
        if (dateIndex != std::string::npos) {
          ...
          std::string now =
              base::utils::DateTime::getDateTime(fmt.c_str(), &msWidth);
          base::utils::Str::replaceAll(now, '/', '-');    // Replace path
                                                          // element since we
                                                          // are dealing with
                                                          // filename
          base::utils::Str::replaceAll(
              resultingFilename, dateTimeFormatSpecifierStr, now);
        }
      }
    ...
    bool validateFileRolling(Level level,
                             const PreRollOutCallback& PreRollOutCallback)
    {
      base::threading::ScopedLock scopedLock(lock());
  ...
  {
  public:
    /// @brief Validates counter for every N, i.e, registers new if does not
    /// exist otherwise updates original one
    /// @return True if validation resulted in triggering hit. Meaning logs
    /// should be written everytime true is returned
    ...
    /// @brief Validates counter for hits are <= n, i.e, registers new if does
    /// not exist otherwise updates original one
    /// @return True if validation resulted in triggering hit. Meaning logs
    /// should be written everytime true is returned
  ...
  void configure(const Configurations& configurations)
  {
    ...
    if (m_typedConfigurations != nullptr) {
      ...
      if (c->hasConfiguration(Level::Global, ConfigurationType::Filename)) {
        // This check is definitely needed for cases like
    ...
    void setFromArgs(const base::utils::CommandLineArgs* commandLineArgs)
    {
      if (commandLineArgs->hasParam("-v") ||
        ...
        setLevel(atoi(commandLineArgs->getParamValue("--V")));
                 ...
                 (!base::utils::hasFlag(LoggingFlag::DisableVModules,
                                        *m_pFlags))) {
...
public:
  LogMessage(Level level,
    ...
    , m_logger(logger)
    , m_message(std::move(logger->stream().str()))
  {
  }
  inline Level level(void) const
  {
...
namespace base
{
  ...
  public:
    ...
    inline void installCustomFormatSpecifier(
  ...
  private:
    ...
    std::map<std::string, base::type::LogDispatchCallbackPtr>
...
#if defined(_ELPP_LOGGING_FLAGS_FROM_ARG)
      if (m_commandLineArgs.hasParamWithValue(
              base::consts::kLoggingFlagsParam)) {
        m_flags = atoi(
...
namespace base
{
  ...
  public:
    ...
    void dispatch(void)
    {
      ...
      // We minimize the time of ELPP's lock - this lock is released after log
      // is written
  ...
  /// order to do that, we need iterable containers
  /// of same type and provide iterator interface and pass it on to
  /// writeIterator().
  /// Remember, this is passed by value in constructor so that we dont change
  /// original containers.
  ...
  namespace workarounds
  {
    ...
    public:
      IterablePriorityQueue(
          std::priority_queue<T, Container, Comparator> queue_)
      {
        std::size_t count_ = 0;
        while (++count_ < base::consts::kMaxLogPerContainer &&
               !queue_.empty()) {
    ...
    /// @brief Implements IterableContainer and provides iterable std::queue
    /// class
    ...
    /// @brief Implements IterableContainer and provides iterable std::stack
    /// class
    template <typename T, typename Container = std::deque<T>>
  ...
  public:
    MessageBuilder(void)
      : m_logger(nullptr)
      , m_containerLogSeperator(ELPP_LITERAL(""))
    {
    }
    void initialize(Logger* logger)
    {
      ...
      m_containerLogSeperator =
          ELPP->hasFlag(LoggingFlag::NewLineForContainer) ?
              ELPP_LITERAL("\n    ") :
              ELPP_LITERAL(", ");
    }
...
#define ELPP_ITERATOR_CONTAINER_LOG_ONE_ARG(temp)                              \
  ...
  {                                                                            \
    return writeIterator(                                                      \
        template_inst.begin(), template_inst.end(), template_inst.size());     \
  }
...
#define ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG(temp)                             \
  ...
  {                                                                            \
    ...
    inline MessageBuilder& operator<<(const std::queue<T, Container>& queue_)
    {
      ...
      return writeIterator(
          iterableQueue_.begin(), iterableQueue_.end(), iterableQueue_.size());
    }
    template <class T, class Container>
    inline MessageBuilder& operator<<(const std::stack<T, Container>& stack_)
    {
      ...
      return writeIterator(
          iterableStack_.begin(), iterableStack_.end(), iterableStack_.size());
    }
        ...
        const std::priority_queue<T, Container, Comparator>& priorityQueue_)
    {
      base::workarounds::IterablePriorityQueue<T, Container, Comparator>
          iterablePriorityQueue_ =
              static_cast<base::workarounds::
                              IterablePriorityQueue<T, Container, Comparator>>(
                  priorityQueue_);
...
/// @brief Macro used internally that can be used externally to make containers
/// easylogging++ friendly
///
...
/// @param ContainerType Type of container e.g, MyList from WX_DECLARE_LIST(int,
/// MyList); in wxwidgets
...
/// "elem". See WX_ELPP_ENABLED macro
///        for an example usage
#define MAKE_CONTAINER_ELPP_FRIENDLY(                                          \
    ContainerType, SizeMethod, ElementInstance)                                \
...
#define ELPP_WX_PTR_ENABLED(ContainerType)                                     \
  MAKE_CONTAINER_ELPP_FRIENDLY(ContainerType, size(), *(*elem))
    ...
    void triggerDispatch(void)
    {
      if (m_proceed) {
        base::LogDispatcher(
            m_proceed,
            LogMessage(
                m_level, m_file, m_line, m_func, m_verboseLevel, m_logger),
      ...
      if (m_proceed && m_level == Level::Fatal &&
        ...
        reasonStream
            ...
            << "el::Helpers::addFlag(el::LoggingFlag::"
...
template <typename T, typename... Args>
void Logger::log_(
    Level level, int vlevel, const char* s, const T& value, const Args&... args)
{
...
inline void Logger::log_(Level level, int vlevel, const T& log)
{
  if (level == Level::Verbose) {
    if (ELPP->vRegistry()->allowed(vlevel, __FILE__)) {
      base::Writer(Level::Verbose,
                   "FILE",
                   0,
                   "FUNCTION",
                   base::DispatchAction::NormalLog,
                   vlevel).construct(this, false)
          << log;
    } else {
...
#define LOGGER_LEVEL_WRITERS(FUNCTION_NAME, LOG_LEVEL)                         \
  template <typename T, typename... Args>                                      \
  inline void Logger::FUNCTION_NAME(                                           \
      const char* s, const T& value, const Args&... args)                      \
  {                                                                            \
...
#define _ELPP_WRITE_LOG(writer, level, dispatchAction, ...)                    \
  writer(level, __FILE__, __LINE__, _ELPP_FUNC, dispatchAction)                \
      .construct(el_getVALength(__VA_ARGS__), __VA_ARGS__)
#define _ELPP_WRITE_LOG_IF(writer, condition, level, dispatchAction, ...)      \
  if (condition)                                                               \
  writer(level, __FILE__, __LINE__, _ELPP_FUNC, dispatchAction)                \
      .construct(el_getVALength(__VA_ARGS__), __VA_ARGS__)
...
#else
#define _CURRENT_FILE_PERFORMANCE_LOGGER_ID                                    \
  el::base::consts::kPerformanceLoggerId
#endif
class PerformanceTrackingData
{
public:
  ...
  public:
    PerformanceTracker(
        ...
        const std::string& loggerId =
            std::string(_CURRENT_FILE_PERFORMANCE_LOGGER_ID),
        bool scopedLog = true,
      ...
      , m_enabled(false)
    {
      ...
      // We store it locally so that if user happen to change configuration by
      // the end of scope
      // or before calling checkpoint, we still depend on state of configuraton
    ...
    {    // NOLINT
      ...
      if (m_enabled) {
        ...
        base::type::string_t formattedTime =
            m_hasChecked ?
                base::utils::DateTime::formatTime(
                    base::utils::DateTime::getTimeDifference(
                        m_endTime, m_lastCheckpointTime, m_timestampUnit),
                    m_timestampUnit) :
                ELPP_LITERAL("");
        PerformanceTrackingData data(
  ...
  private:
    ...
    const base::type::string_t getFormattedTimeTaken() const
    {
      return base::utils::DateTime::formatTime(
          base::utils::DateTime::getTimeDifference(
              m_endTime, m_startTime, m_timestampUnit),
          m_timestampUnit);
    }
...
namespace base
{
  ...
  namespace debug
  {
      ...
      void generateNew(void)
      {
        ...
        if (size > kStackStart) {    // Skip StackTrace c'tor and generateNew
          for (std::size_t i = kStackStart; i < size; ++i) {
            ...
            if (mangName != nullptr && hex != nullptr && addr != nullptr &&
                mangName < hex) {
              *mangName++ = '\0';
              *hex++ = '\0';
              *addr++ = '\0';
      ...
      for (int i = 0; i < base::consts::kCrashSignalsCount; ++i) {
        if (base::consts::kCrashSignals[i].numb == sig) {
          ...
          if (ELPP->hasFlag(el::LoggingFlag::LogDetailedCrashReason)) {
            ss << std::endl << "    " << base::consts::kCrashSignals[i].brief
               << std::endl << "    " << base::consts::kCrashSignals[i].detail;
          }
          foundReason = true;
        }
      }
...
#endif    // _ELPP_STACKTRACE
      _ELPP_WRITE_LOG(
          el::base::Writer, level, base::DispatchAction::NormalLog, logger)
          << ss.str();
    }
    ...
    ///
    /// @detail This function writes log using 'default' logger, prints stack
    /// trace for GCC based compilers and aborts program.
    static inline void defaultCrashHandler(int sig)
    {
      base::debug::logCrashReason(
          sig, true, Level::Fatal, base::consts::kDefaultLoggerId);
...
#define _INIT_SYSLOG(id, opt, fac)                                             \
...
public:
  ...
  static inline Logger* getLogger(const std::string& identity,
  ...
  static inline Logger* reconfigureLogger(Logger* logger,
                                          const Configurations& configurations)
  {
    if (!logger)
      return nullptr;
  ...
  /// level
  static inline void reconfigureAllLoggers(Level level,
                                           ConfigurationType configurationType,
                                           const std::string& value)
  {
    for (base::RegisteredLoggers::iterator it =
  ...
  populateAllLoggerIds(std::vector<std::string>* targetList)
  {
    ...
    for (base::RegisteredLoggers::iterator it =
         ...
         it != ELPP->registeredLoggers()->list().end();
         ++it) {
  ...
  ///         successfully, it only means that it
  ///         has attempeted to configure logger using configuration file
...
public:
  ...
  {
    return std::string("9.75");
  }
  /// @brief Release date of current version
  static inline const std::string releaseDate(void)
  {
    return std::string("06-08-2014 1303hrs");
  }
};
}    // namespace el
#undef VLOG_IS_ON
...
#define TIMED_SCOPE(obj, blockname)                                            \
  el::base::PerformanceTracker obj(blockname, _ELPP_MIN_UNIT)
#define TIMED_BLOCK(obj, blockName)                                            \
  for (struct {                                                                \
    int i;                                                                     \
    el::base::PerformanceTracker timer;                                        \
       } obj = {0, el::base::PerformanceTracker(blockName, _ELPP_MIN_UNIT)};   \
       obj.i < 1;                                                              \
       ++obj.i)
...
#define PERFORMANCE_CHECKPOINT(obj)                                            \
  obj.checkpoint(std::string(), __FILE__, __LINE__, _ELPP_FUNC)
#define PERFORMANCE_CHECKPOINT_WITH_ID(obj, id)                                \
  obj.checkpoint(id, __FILE__, __LINE__, _ELPP_FUNC)
#undef ELPP_COUNTER
...
#define CINFO_IF(writer, condition_, dispatchAction, ...)                      \
  _ELPP_WRITE_LOG_IF(                                                          \
      writer, (condition_), el::Level::Info, dispatchAction, __VA_ARGS__)
#else
...
#define CWARNING_IF(writer, condition_, dispatchAction, ...)                   \
  _ELPP_WRITE_LOG_IF(                                                          \
      writer, (condition_), el::Level::Warning, dispatchAction, __VA_ARGS__)
#else
#define CWARNING_IF(writer, condition_, dispatchAction, ...)                   \
  el::base::NullWriter()
#endif    // _ELPP_WARNING_LOG
...
#define CDEBUG_IF(writer, condition_, dispatchAction, ...)                     \
  _ELPP_WRITE_LOG_IF(                                                          \
      writer, (condition_), el::Level::Debug, dispatchAction, __VA_ARGS__)
#else
#define CDEBUG_IF(writer, condition_, dispatchAction, ...)                     \
  el::base::NullWriter()
#endif    // _ELPP_DEBUG_LOG
...
#define CERROR_IF(writer, condition_, dispatchAction, ...)                     \
  _ELPP_WRITE_LOG_IF(                                                          \
      writer, (condition_), el::Level::Error, dispatchAction, __VA_ARGS__)
#else
#define CERROR_IF(writer, condition_, dispatchAction, ...)                     \
  el::base::NullWriter()
#endif    // _ELPP_ERROR_LOG
...
#define CFATAL_IF(writer, condition_, dispatchAction, ...)                     \
  _ELPP_WRITE_LOG_IF(                                                          \
      writer, (condition_), el::Level::Fatal, dispatchAction, __VA_ARGS__)
#else
#define CFATAL_IF(writer, condition_, dispatchAction, ...)                     \
  el::base::NullWriter()
#endif    // _ELPP_FATAL_LOG
...
#define CTRACE_IF(writer, condition_, dispatchAction, ...)                     \
  _ELPP_WRITE_LOG_IF(                                                          \
      writer, (condition_), el::Level::Trace, dispatchAction, __VA_ARGS__)
#else
#define CTRACE_IF(writer, condition_, dispatchAction, ...)                     \
  el::base::NullWriter()
#endif    // _ELPP_TRACE_LOG
...
#define CINFO_EVERY_N(writer, occasion, dispatchAction, ...)                   \
  _ELPP_WRITE_LOG_EVERY_N(                                                     \
      writer, occasion, el::Level::Info, dispatchAction, __VA_ARGS__)
#else
#define CINFO_EVERY_N(writer, occasion, dispatchAction, ...)                   \
  el::base::NullWriter()
#endif    // _ELPP_INFO_LOG
...
#define CWARNING_EVERY_N(writer, occasion, dispatchAction, ...)                \
  _ELPP_WRITE_LOG_EVERY_N(                                                     \
      writer, occasion, el::Level::Warning, dispatchAction, __VA_ARGS__)
#else
#define CWARNING_EVERY_N(writer, occasion, dispatchAction, ...)                \
  el::base::NullWriter()
#endif    // _ELPP_WARNING_LOG
...
#define CDEBUG_EVERY_N(writer, occasion, dispatchAction, ...)                  \
  _ELPP_WRITE_LOG_EVERY_N(                                                     \
      writer, occasion, el::Level::Debug, dispatchAction, __VA_ARGS__)
#else
#define CDEBUG_EVERY_N(writer, occasion, dispatchAction, ...)                  \
  el::base::NullWriter()
#endif    // _ELPP_DEBUG_LOG
...
#define CERROR_EVERY_N(writer, occasion, dispatchAction, ...)                  \
  _ELPP_WRITE_LOG_EVERY_N(                                                     \
      writer, occasion, el::Level::Error, dispatchAction, __VA_ARGS__)
#else
#define CERROR_EVERY_N(writer, occasion, dispatchAction, ...)                  \
  el::base::NullWriter()
#endif    // _ELPP_ERROR_LOG
...
#define CFATAL_EVERY_N(writer, occasion, dispatchAction, ...)                  \
  _ELPP_WRITE_LOG_EVERY_N(                                                     \
      writer, occasion, el::Level::Fatal, dispatchAction, __VA_ARGS__)
#else
#define CFATAL_EVERY_N(writer, occasion, dispatchAction, ...)                  \
  el::base::NullWriter()
#endif    // _ELPP_FATAL_LOG
...
#define CTRACE_EVERY_N(writer, occasion, dispatchAction, ...)                  \
  _ELPP_WRITE_LOG_EVERY_N(                                                     \
      writer, occasion, el::Level::Trace, dispatchAction, __VA_ARGS__)
#else
#define CTRACE_EVERY_N(writer, occasion, dispatchAction, ...)                  \
  el::base::NullWriter()
#endif    // _ELPP_TRACE_LOG
...
#define CINFO_AFTER_N(writer, n, dispatchAction, ...)                          \
  _ELPP_WRITE_LOG_AFTER_N(                                                     \
      writer, n, el::Level::Info, dispatchAction, __VA_ARGS__)
#else
...
#define CWARNING_AFTER_N(writer, n, dispatchAction, ...)                       \
  _ELPP_WRITE_LOG_AFTER_N(                                                     \
      writer, n, el::Level::Warning, dispatchAction, __VA_ARGS__)
#else
...
#define CDEBUG_AFTER_N(writer, n, dispatchAction, ...)                         \
  _ELPP_WRITE_LOG_AFTER_N(                                                     \
      writer, n, el::Level::Debug, dispatchAction, __VA_ARGS__)
#else
...
#define CERROR_AFTER_N(writer, n, dispatchAction, ...)                         \
  _ELPP_WRITE_LOG_AFTER_N(                                                     \
      writer, n, el::Level::Error, dispatchAction, __VA_ARGS__)
#else
...
#define CFATAL_AFTER_N(writer, n, dispatchAction, ...)                         \
  _ELPP_WRITE_LOG_AFTER_N(                                                     \
      writer, n, el::Level::Fatal, dispatchAction, __VA_ARGS__)
#else
...
#define CTRACE_AFTER_N(writer, n, dispatchAction, ...)                         \
  _ELPP_WRITE_LOG_AFTER_N(                                                     \
      writer, n, el::Level::Trace, dispatchAction, __VA_ARGS__)
#else
...
#define CLOG_EVERY_N(n, LEVEL, ...)                                            \
  C##LEVEL##_EVERY_N(                                                          \
      el::base::Writer, n, el::base::DispatchAction::NormalLog, __VA_ARGS__)
#define CVLOG_EVERY_N(n, vlevel, ...)                                          \
  CVERBOSE_EVERY_N(el::base::Writer,                                           \
                   n,                                                          \
                   vlevel,                                                     \
                   el::base::DispatchAction::NormalLog,                        \
                   __VA_ARGS__)
...
#define CVLOG_AFTER_N(n, vlevel, ...)                                          \
  CVERBOSE_AFTER_N(el::base::Writer,                                           \
                   n,                                                          \
                   vlevel,                                                     \
                   el::base::DispatchAction::NormalLog,                        \
                   __VA_ARGS__)
...
#define CVLOG_N_TIMES(n, vlevel, ...)                                          \
  CVERBOSE_N_TIMES(el::base::Writer,                                           \
                   n,                                                          \
                   vlevel,                                                     \
                   el::base::DispatchAction::NormalLog,                        \
                   __VA_ARGS__)
...
#undef DPLOG_IF
#define CPLOG(LEVEL, ...)                                                      \
  C##LEVEL(el::base::PErrorWriter,                                             \
           el::base::DispatchAction::NormalLog,                                \
           __VA_ARGS__)
#define CPLOG_IF(condition, LEVEL, ...)                                        \
  C##LEVEL##_IF(el::base::PErrorWriter,                                        \
                condition,                                                     \
                el::base::DispatchAction::NormalLog,                           \
                __VA_ARGS__)
#define DCPLOG(LEVEL, ...)                                                     \
...
#define CSYSLOG_EVERY_N(n, LEVEL, ...)                                         \
  C##LEVEL##_EVERY_N(                                                          \
      el::base::Writer, n, el::base::DispatchAction::SysLog, __VA_ARGS__)
...
#define CCHECK_STRCASEEQ(str1, str2, ...)                                      \
  CLOG_IF(                                                                     \
      !el::base::utils::Str::cStringCaseEq(str1, str2), FATAL, __VA_ARGS__)    \
      << "Check failed: [" << #str1 << " == " << #str2 << "] "
...
#define CHECK_STREQ(str1, str2)                                                \
  CCHECK_STREQ(str1, str2, _CURRENT_FILE_LOGGER_ID)
#define CHECK_STRNE(str1, str2)                                                \
  CCHECK_STRNE(str1, str2, _CURRENT_FILE_LOGGER_ID)
#define CHECK_STRCASEEQ(str1, str2)                                            \
  CCHECK_STRCASEEQ(str1, str2, _CURRENT_FILE_LOGGER_ID)
#define CHECK_STRCASENE(str1, str2)                                            \
  CCHECK_STRCASENE(str1, str2, _CURRENT_FILE_LOGGER_ID)
#undef DCCHECK
#undef DCCHECK_EQ
#undef DCCHECK_NE
#undef DCCHECK_LT
...
#define DCHECK_STREQ(str1, str2)                                               \
  DCCHECK_STREQ(str1, str2, _CURRENT_FILE_LOGGER_ID)
#define DCHECK_STRNE(str1, str2)                                               \
  DCCHECK_STRNE(str1, str2, _CURRENT_FILE_LOGGER_ID)
#define DCHECK_STRCASEEQ(str1, str2)                                           \
  DCCHECK_STRCASEEQ(str1, str2, _CURRENT_FILE_LOGGER_ID)
#define DCHECK_STRCASENE(str1, str2)                                           \
  DCCHECK_STRCASENE(str1, str2, _CURRENT_FILE_LOGGER_ID)
...
#define _ELPP_INIT_EASYLOGGINGPP(val)                                          \
  _ELPP_INITI_BASIC_DECLR namespace el                                         \
  {                                                                            \
    namespace base                                                             \
    {                                                                          \
...
#define _INITIALIZE_EASYLOGGINGPP                                              \
  _ELPP_INIT_EASYLOGGINGPP(new el::base::Storage(                              \
...
// NOTE: no _ELPP_INITI_BASIC_DECLR when sharing - causes double free corruption
// on external symbols
