...
struct Task_ final {
  ...
  std::function<void()> f;
  // Called by the scheduler at the end of execution to update the task's state
...
private:
  ...
  Task(F&& f, Args&&... args)
  {
    ...
    static_assert(std::is_same<T, return_type>,
    ...
    task_ = Task_{[binding]() { (*binding)(); }, [this]() { complete(); }};
  }
...
{
public:
  template <typename F, typename... Args, typename Delay>
  auto schedule(Delay delay, F&& f, Args&&... args)
      -> std::shared_ptr<Task<std::result_of<F(Args)>>>
  {
    using return_type = typename std::result_of<F(Args...)>::type;

    auto task = Task<return_type>{std::forward<F>(f), std::forward<Args>(args)...};
  }
};
}