...
void Segmenter::storeFile (std::string fileName, std::vector<ChunkID>& chunks, DataLayer& datalayer)
{
  std::ifstream fin(fileName.c_str(), std::ios_base::ate|std::ios::binary|std::ios::in);
  if (fin.is_open())
  {
...
//              << "Size: " << nSize << std::endl; 
    ...
    while (fin)
    {
      ...
      int size_to_read = ((currPointer + nChunkSize > nSize) ? nSize % nChunkSize : nChunkSize);
      char* memBlock = new char[size_to_read];
      fin.read(memBlock, size_to_read);
      std::string exportFileName;
      exportFileName.append(fileName);
      exportFileName.append(".part"); //should be relocated to the local cache.
...
//      std::cout << "Exporting chunk: " << exportFileName << std::endl;
      ...
      chunks.push_back(id);
      // store to ChunkStore
      datalayer.storeChunk(exportFileName, false); // not from network, but from local application layer..
...
//    std::cout << "Can't open the so-store file..." << fileName << std::endl;
    logger->log (oss.str());
  }
} 

void Segmenter::loadFileFromChunks (std::string assembledFileName, const std::vector<ChunkID>& chunks, const DataLayer& datalayer) const
{
