...
void Emulator::processCommand(std::string cmdLine)
{
  ...
  if (argv[0] == "create")
  {
    ...
    if(argv[1] == "network")
    {
      std::transform(argv[2].begin(), argv[2].end(), argv[2].begin(), ::tolower);
      assert(argv[2] == "adhoc");
    ...
    else if (argv[1] == "device")
    {
      ...
      if (network == NULL)
      {
        std::cout << "Error when create devices: no network available to connect. " << std::endl;
        return;
      }
...
//      mobileDevice->connectToNetwork(network);
      devices.insert(make_pair(deviceID, mobileDevice)); // keep track of the devices created.
    }
    else
    {
      std::cout << "Invalid Command: only network or device can be created." << std::endl;
    }   
  } 
  else if (argv[0] == "setgps")
  {
    assert(argv.size() == 4);
    DeviceWithGPS* deviceHandler = (DeviceWithGPS*) (this->getDeviceByID(argv[1]));
...
{
  std::unordered_map<std::string, Device*>::const_iterator iter = devices.find(id);
  if (iter == devices.end())
  {
...
void Emulator::clearObject()
{
  ...
  std::unordered_map<std::string, Device*>::const_iterator iter = devices.begin();
