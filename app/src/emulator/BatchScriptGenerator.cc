...
std::string BatchScriptGenerator::DEFAULT_SCRIPT_FILE_NAME = "./build/emulationInbatch.txt";
int BatchScriptGenerator::DEFAULT_NUM_OF_NODES = 1000;

BatchScriptGenerator::BatchScriptGenerator (std::string fileName):scriptFileName(fileName)
{
  srand(time(NULL));
  generateTopology();
  generateActivity();
...
void BatchScriptGenerator::generateTopology()
{
  ...
  for (int i=0; i<DEFAULT_NUM_OF_NODES; i++)
  {
    ...
    fout << "setgps " << i << " " << randomLatitude() << " " << randomLongitude() << std::endl;
